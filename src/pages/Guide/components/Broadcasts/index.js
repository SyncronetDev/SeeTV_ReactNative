...
import Event from '../Broadcast';

import styles from './index.styles';
import HeaderStyle from '../ChannelHeader/index.styles';
...
const CONTENT_HEIGHT = constants.ROW_HEIGHT * TIME_LABELS_COUNT;
const TIME_LABEL_WIDTH = constants.CHANNELHEADER.width;
...
class Broadcasts extends Component {
  ...
  getStyleForEvent = (item) => {
    ...
    const height =
      (moment(item.endTime).diff(item.startTime, 'minutes') * CONTENT_HEIGHT) / MINUTES_IN_DAY;
    const width = this.getEventItemWidth();

    return {
      top: topOffset + constants.CONTENT_OFFSET,
      left: 0,
      height,
      width,
    };
  ...
  render() {
    const { events, numberOfDays, selectedDate, times } = this.props;
